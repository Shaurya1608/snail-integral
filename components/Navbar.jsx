        ...
        <div className="flex-1 flex flex-col px-8 pt-12 gap-6 overflow-y-auto">
          ...
          >
            ...
            >
              Events
              <svg
                className={`w-6 h-6 text-gray-300 group-hover:text-primary transition-all duration-300 ${mobileEventsOpen ? 'rotate-90' : ''}`}
                fill="none" stroke="currentColor" viewBox="0 0 24 24"
              >
            ...
            {/* Sub-items */}
            <div className={`mt-4 flex flex-col gap-3 overflow-hidden transition-all duration-300 ${mobileEventsOpen ? 'max-h-60 opacity-100' : 'max-h-0 opacity-0'}`}>
              {events.map((event) => (
                <Link
                  key={event.label}
                  href={event.href}
                  onClick={() => { setMobileOpen(false); setMobileEventsOpen(false); }}
                  className="flex items-center gap-3 pl-4 py-2 rounded-xl hover:bg-white group transition-all"
                >
                  <div className="w-12 h-9 flex items-center justify-center bg-white rounded-lg border border-gray-100 p-1.5 flex-shrink-0 shadow-sm">
                    <Image
                      src={event.logo}
                      alt={event.label}
                      width={40}
                  ...
                  <div>
                    <div className="text-base font-semibold text-gray-700 group-hover:text-[#4a7c24] transition-colors">{event.label}</div>
